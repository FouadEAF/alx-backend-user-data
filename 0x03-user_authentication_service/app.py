#!/usr/bin/env python3
""" API Route for Authentication Service """
...
def register_user() -> str:
    """ Register a new user if it does not exist before """
    try:
...
def log_in() -> str:
    """ Login a user and return session ID """
    try:
        email = request.form['email']
        password = request.form['password']
    except KeyError:
...
def profile() -> str:
    """ If the user exist, respond with a 200 HTTP status
        and a JSON Payload Otherwise respond with a 403 HTTP status.
    """
    session_id = request.cookies.get("session_id", None)
